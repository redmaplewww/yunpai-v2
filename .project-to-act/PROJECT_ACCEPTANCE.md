...
| E-008 | 2026-09-13 |**M6 增量（B0a + B0b + R0 + R1a + B1-1 + B1-2 + B1-3 + B2 + B3 + B4 + 跨线收口 + B5 + B6）**：CANONICAL_SCHEMA 收口 `expense`/`delivery_note`/`stock_class`(四态)+`workshop`+`amount`；`m6_defaults.py`(83)/`m6_cost.py`(601)/`m6_price_source.py`(含 D5 `resolve_material_prices`)/`m6_store.py`/`m6_tools.py` 落位；`registry-manifests/m6.json` 24 契约（成本账 9 + 单据 6 + 凭据 2 + 资产 3 + 库存财务视图 1 + 订单/工资 3）+ `finance` 门型入 gates.py + `_REVIEW_GATE_MAP` 补 `finance`；B1 第三批抽 `_cost_breakdown` 共用预算（save 与 get_product_cost 同口径，测试断言单位成本相等）+ `_assemble_costing_facts` 共用装配；`graph.py` 四个 `_apply_m6_*` commit 钩子（approve 分支分派、冲突不抛异常）+ `state.review_applied`；`orchestration_bridge` 装配接线（含 R1a `read_m4_tracking_price_facts` 读 M4B 追踪价源）；`ENTITY_TYPES`+2、`FACADE_KINDS`+2、`list_expenses`/`list_delivery_notes`（并修掉信封未归一导致恒空的 bug）、m0.json+4 契约、RULES 门+2；新增 `test_m6_cost`(15)/`test_m6_expense`(9)/`test_m6_entities`(6)/`test_m6_price_source`(6)/`test_m6_store`(14)/`test_m6_costing_tools`(21)/`test_m6_finance_gate`(7)/`test_m6_entities_roundtrip`(3)，`test_m6_cost_tools`(14)/`test_m6_documents`(13)/`test_m6_delivery`(7)/`test_m6_assets`(7)，`test_binding` 计数锁 119→123→129→132→138→140→143→144→147。**B7（D-011）**：`m6_tooling.py`（两个 Skill 的 operation map 单一真源）+ `skills.py` 两个 handler 与两条 `SkillSpec` + `skills/m6-finance/`·`skills/m6-ledger/`（各 `SKILL.md` + `references/tools.md`）+ `SKILL_USAGE_ORDER` 末位追加 + `test_m6_skills.py`(23)（操作面覆盖/不相交、default 纯读、白名单外 `tool` 一律 `ValueError`、G2 载荷、文档漂移）+ `test_skill_registry_consistency` 直取真源常量（去镜像）+ legacy skill catalog 精确集 8→10。pytest **825 passed/3 skipped**（基线 661/2）；check_contracts exit 0（工具 119→147、handler 100→128；**skill 不计入工具面**）—— 三段式 5 条验收断言（说明一 §1.7）全过：trial 不进汇总 / finance 门 interrupt→approve 才翻 confirmed / reject 不产生 confirmed 行 / 越权角色被拦且不抛异常 / 无自动放行 |通过（增量；F-008 计划批次已走完，余 2 件受 R1b 阻塞） | 分支 `feat/m6-finance-20260913`（提交 1d796ce / ed4d32c / 5531c08 + 各批次）；范围与口径见 `M6-三项决策说明-20260913.md`、D-008/D-011 见 `PROJECT_OVERVIEW.md` | B-013 已决策并执行首批；B-014 已关闭（D-010 收窄为 M6 专属）；供应商报价 2 件 ⛔ 阻塞 R1b（**非批次遗留**）；`stock_class` 枚举值来源为空（数据侧）；`standard_time` 单位口径未定（`_standard_minutes_of` 已在仓库但 M6 未接线）；老仓四张工资 canonical 面 v2 全无；`inventory`/`production_daily_report` 等 6 类不在 `ENTITY_TYPES`（M0 口径）；币种/含税未换算；老仓 7 个工具级 registry 用例随其工具批次移植 | 至 M6 全批次完成 |
...
| 2026-09-15 | M6 文件规范收敛增量：对账显式交易缺金额/方向 fail-closed；资产零基准标记 `missing_basis_value`；M6 `m6_db_path` 条件透传；Tool 返回增加 `result/error/business_status` 并同步 M6 output schema；新增 3 条回归测试 | E-009 | 通过（M6 165 passed/1 skipped；全仓 828 passed/3 skipped；check_contracts exit 0；JSON Schema 全部有效；`git diff --check` 通过；D-008 刻意保留 1 条 W2，因此 `--strict` 按当前检查器会失败） | 全局 Data Manager / Domain Service / Repository 与 Tool/Skill 文件重命名仍待 main 架构冻结 | P-015 已完成（`ce61cae`） |
| 2026-09-15 | M6 夹具化独立测试与口径修复：7 组带 `source_ref`/`evidence` 的 JSON 夹具；缺数量、空工资事实、财务角色、commit `data/result` 一致性回归 | E-010 | 通过（全仓 834 passed/5 skipped；新增夹具测试 7 项；身份/门禁聚焦 24 passed；check_contracts exit 0；compileall 通过） | 真实 canonical 工资事实尚未接入；身份登记不等于 API 强制授权；全局架构命名迁移等待 main 冻结 | P-016 已完成（`ce61cae`）；复核发现门载荷缺审阅内容、API 层零覆盖 → 立 P-017 |
| 2026-09-15 | M6 P-017 补充轮次：finance 门审阅摘要 + 真实 `create_app` API 夹具（T6/T7） | E-011 | 通过（T6/T7 聚焦 **46 passed**；`-k m6` **179 passed/1 skipped**；全量 **841 passed/5 skipped**；`check_contracts.py` exit 0；覆盖 `/runs` 创建、finance approve/reject、operator 越权、请求头角色、`public_state` 脱敏与 review 金额；T6 旧 Gate 7 键兼容；`py_compile` 通过） | 租户隔离 T-51、真实工资 canonical 面仍属遗留；测试过程有 FastAPI/aiosqlite 非阻断弃用及线程清理告警 | P-017 已完成；API `pending_gate` 采用现有 `public_state` 直接门对象形状 |
