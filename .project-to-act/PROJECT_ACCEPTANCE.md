...
| E-008 | 2026-09-13 | **M6 增量（B0a + R0 + B0b）**：CANONICAL_SCHEMA 收口 `expense`/`delivery_note`/`stock_class`(四态)+`workshop`+`amount`；`m6_defaults.py`(83)/`m6_cost.py`(601) 落位；`ENTITY_TYPES`+2、`FACADE_KINDS`+2、`list_expenses`/`list_delivery_notes`、m0.json+4 契约、RULES 门+2；新增 `test_m6_cost.py`(15)/`test_m6_expense.py`(9)/`test_m6_entities.py`(6)，`test_binding.py` 计数锁 119→123。pytest **692 passed/2 skipped**（基线 661/2）；check_contracts exit 0（工具 119→123、handler 100→104） | 通过（增量；F-008 进行中） | 分支 `feat/m6-finance-20260913` 基线 b87792b；范围与口径见 `M6-三项决策说明-20260913.md` | B-013 已决策并执行首批；B-014 待排期；**canonical 导入→回读往返未验**（需 `m0.ingest.v1` 完整信封，随 B1 补）；`注册审查台账` 未登记新增 4 件；书二 §6.2.1 三段式条文已补 | 至 M6 全批次完成 |
