...
| F-008 | M6 财务：成本核算链 + 成本明细账 | M6-开发计划-v2口径-20260913.md §0 | P0 | 进行中 | 增量验收（E-008） | F-004 | 成本链可按 D5/D6 口径算出并落快照；三段式（先批准后落库）生效；试算/正式分离；工具面 **28/28 已注册**（B0b 4 + B1 9 + B2 6 + B3 2 + B4 3 + B5 1 + B6 3）；**计划 §5.2 的 28 件清单完成 26 件**（余 2 件供应商报价 ⛔ 阻塞 R1b）；**两个 Skill 已注册**（技能 8→10，`yunpai-m6-finance` 算 / `yunpai-m6-ledger` 记） | M6-开发计划 §3–§7 | E-008 |
...
| 2026-09-14 | F-008 | 第五批增量（**B3 凭据**）：`get_delivery_note`（读 canonical 送货单）+ `generate_statement`（依据送货单/入库生成对账明细，客户/供应商双向），`save_statement` 因此可自动取依据（工具面 138→**140**） | 凭据层补齐：对账不再要求人手工抄 transactions；**依据缺失即拒出单**（basis_source=missing，绝不给零金额空对账单）；供应商侧不把追踪行的 `unit_price`（单价）当入库金额（依据行须自带 amount）；读口**必须拆信封**由真 canonical 往返测试锁住（B0b 同类 bug 不再重演） | E-008 | Agent（按 D4／老仓 D-020 = v2 D-009 与 D9 实现） |
| 2026-09-14 | F-008 | 第六批增量（**B4 资产台账**）：`upsert_asset_ledger`（按 `asset_code+revision` 追加 trial 修订 → `finance` 门 → 翻正）/ `get_asset_ledger`（生效值与草稿分列）/ `compute_asset_benefit`（成本取台账生效原值或显式给）（工具面 140→**143**） | 台账也必须"先批准后落库"：propose **绝不改写已确认原值**（修订模型），未批草稿不参与成本分摊；缺原值/缺成本一律 `missing` + `cost_incomplete`；顺带写明 schema 版本政策（纯新增表不 bump，靠 `CREATE TABLE IF NOT EXISTS` 补齐） | E-008 | Agent（按 D-005 三段式实现） |
| 2026-09-14 | F-008 | 第七批增量（**B5 库存财务视图 + B6 订单/工资**，工具面 143→**147**）：`get_inventory_finance_view`（四态分账 + 在途）、`list_orders`、`calculate_piece_pay`、`calculate_monthly_pay`（4 件纯算数读、无门） | 库存视图三条全部 fail-closed（态缺失/未知→`unknown` **不归任何一态**、缺单价→`cost_incomplete` **不编造成本**、追踪行 `arrival_status` 空→`unknown_status` **不猜在途**）；**顺带修一个真缺陷**：`read_inventory_facts` 的 canonical 投影只带 6 字段、**丢掉 `stock_class`** → 所有 canonical 库存都成「态未知」，**D5「有库存且态=raw 走库存成本价」即使数据侧补字段也永不可达**；工资两件发现 **v2 无 canonical 工资事实面**（老仓 `usage_log`/`piece_rate`/`salary_standard`/`attendance_summary` 四张面全无）→ 四类事实只能显式给，且**不拿 `production_daily_report` 顶替报工**（老仓明确禁止把 `quantity` 当报工数量），「口径缺省不得静默归零」由回归测试锁定 | E-008 | Agent（按 D-005/D-006 口径实现） |
| 2026-09-14 | F-008 | 第八批增量（**B7 两个 Skill 新建**，技能 8→**10**）：`yunpai-m6-finance`（算：10 件）/ `yunpai-m6-ledger`（记：14 件）+ 契约收口（`m6_tooling.py` operation map 单一来源、`SKILL_USAGE_ORDER` 末位追加、文档漂移门） | Skill 侧也「单一来源」：handler 与 `SkillSpec.tools` 都从 `m6_tooling.py` 的 map 派生，测试**直接 import 常量**对齐（不进镜像表）；**红线由代码兜住**——map 取值集合即全部可调用面，越界 `tool` 一律 `ValueError`（已钉「两个 Skill 都调不到 M0/M5 工具」）；两个 map 的 `default` **都是纯读工具** → 漏写 operation 绝不写库/开门；两 map 不相交且并集 == `m6.json` 的 24 件（无孤儿工具） | E-008 | Agent（按 G2 实装载荷契约 + 任务书 §7；`list_orders` 归属见 D-011） |
